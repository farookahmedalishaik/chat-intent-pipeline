...
def load_evaluation_data():
    ...
    # Correct: Download test_data.pt from Hugging Face Hub
    test_data_file = hf_hub_download(repo_id=hf_model_repo, filename="test_data.pt")
    data = torch.load(test_data_file)
    
    # Correct: Return all three necessary items
    return test_metrics, confusion_matrix, data

# --- FIX 1: UNPACK ALL THREE RETURNED VALUES ---
...
st.sidebar.markdown(INTENT_GUIDANCE_TEXT)
...
input_message = st.text_area("Type your message here:", height=150, label_visibility="collapsed")
...
# --- FIX 2: REMOVE THE INCORRECT LOCAL LOAD AND USE THE CACHED DATA ---
texts = test_data.get("texts", None)
